...
{
  // check if we're multi-threaded
  if(MT)
  {
    // ASSUME: doing exactly the same as below, but in multiple threads
  ...
  // otherwise we're single-threaded
  else
  {
    // reset the sum-squared erros vector
    accSSE_top_A->setZero(nFrames);

    // process each keyframe in window
    for(EFFrame* f : frames)
    {
      // process each keypoint in keyframe
      for(EFPoint* p : f->points)
      {
        // =====================================
        // TODO: review ========================
    ...
    // record total number of residuals
    resInA = accSSE_top_A->nres[0];
  }
}
...
void EnergyFunctional::solveSystemF(int iteration, double lambda,
    CalibHessian* HCalib)
{
  // enforce GN solver, if requested
  if(setting_solverMode & SOLVER_USE_GN) lambda=0;

  // enforce fixed LM lambda, if requested
  if(setting_solverMode & SOLVER_FIX_LAMBDA) lambda = 1e-5;

  // assert valid state of solver
  assert(EFDeltaValid);
  assert(EFAdjointsValid);
  assert(EFIndicesValid);

  // allocate hessian blocks
  MatXX HL_top, HA_top, H_sc;

  // allocate "gradient" (Jtr) blocks
  ...
  // check if orthogonalization requested
  // I assume this has to the gauge freedom of the first pose
  // ASSUME: its forcing the initial trajectory to be axis-aligned
  if (setting_solverMode & SOLVER_ORTHOGONALIZE_SYSTEM)
  {
    // initialize "first-frame" status
    bool haveFirstFrame = false;

    // process each keyframe
    for(EFFrame* f : frames)
    {
      // check if very first keyframe in stream
      if(f->frameID==0)
      {
        // update "first-frame" status
        haveFirstFrame=true;
      }
    }

    // allocate temporary hessian and gradient blocks
    MatXX HT_act = HL_top + HA_top - H_sc;
    VecX bT_act =  bL_top + bA_top - b_sc;

    // check if we have the very first frame
    if(!haveFirstFrame)
    {
      // orthogonalize with first-frame as needed
    ...
    // record new hessian and gradient state
    lastHS = HFinal_top;
    lastbS = bFinal_top;

    // update hessian with LM lambda
  ...
  // allocate update vector
  VecX x;

  // if using SVD to solve problem
  ...
  // assign current state of GN lambda
