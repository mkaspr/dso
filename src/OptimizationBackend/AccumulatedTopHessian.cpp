    ...
    EnergyFunctional const* const ef, int tid)
{
  ...
  // initialize gradient (Jtr) for current keypoint
  float bd_acc=0;

  // initialize SLAM hessian (JtJ) for current keypoint
  float Hdd_acc=0;

  // initialize self-cal hessian (JtJ) for current keypoint
  VecCf Hcd_acc = VecCf::Zero();

  // process each residual (observation) for given keypoint
  for(EFResidual* r : p->residualsAll)
  {
    // if we're only processing active keypoints
    if(mode==0)
    {
      // skip linearized and inactive points
      if(r->isLinearized || !r->isActive()) continue;
    }

    // if we're only processing linearized keypoints
    if(mode==1)
    {
      // skip non-linearized and inactive points
      if(!r->isLinearized || !r->isActive()) continue;
    }

    // if we're only processing marginalized points
    if(mode==2)
    {
      // skip all inactive points
      if(!r->isActive()) continue;

      // confirm point is linearized
      assert(r->isLinearized);
    }

    // get running jacobian for current keypoint
    RawResidualJacobian* rJ = r->J;

    // get index of this output block
    int htIDX = r->hostIDX + r->targetIDX*nframes[tid];

    // TODO: review
    Mat18f dp = ef->adHTdeltaF[htIDX];

    // allocate approximate residual vector
    // TODO: review purpose
    VecNRf resApprox;
    ...
    // check if marginalized mode
    if(mode==2)
    {
      resApprox = r->res_toZeroF;
    }

    // check if linearized mode
    if(mode==1)
    {
      // compute Jp*delta
  ...
  if(mode==0)
  {
    // copy local system variables to keypoint member variables
    p->Hdd_accAF = Hdd_acc;
    p->bd_accAF  = bd_acc;
    p->Hcd_accAF = Hcd_acc;
  }

  // check if linearized or marginalized mode
  if(mode==1 || mode==2)
  {
    // copy local system variables to keypoint member variables
    p->Hdd_accLF = Hdd_acc;
  ...
  if(mode==2)
  {
    // zero out marginalized keypoint
    p->Hcd_accAF.setZero();
    p->Hdd_accAF = 0;
    p->bd_accAF = 0;
  }
